...
export async function searchArtistsByTag(tag: string, limit = 6) {
  return searchArtists(`tag:"${tag}"`, limit);
}
...
export async function searchRecordings(query: string, limit = 10): Promise<MusicBrainzRecording[]> {
  const url = `${MUSICBRAINZ_BASE_URL}/recording?query=${encodeURIComponent(query)}&fmt=json&inc=artist-credits+releases&limit=${limit}`;

  try {
    const response = await rateLimitedFetch(url);
    if (!response.ok) throw new Error("Failed to fetch recordings");

    const data: SearchResult<MusicBrainzRecording> = await response.json();
...
export function getCoverArtUrl(
  ...
  size: "small" | "large" | "250" | "500" | "1200" = "500",
): string {
